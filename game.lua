...
WeaponBlocksRegistry = {}
local dumpCord = { x= -192, y= -192, z= -96}
Game.Weapon:CreateAndDrop(Common.WEAPON.USP45,dumpCord)
for i, pos in pairs(WeaponBlocks) do
	local block = Game.EntityBlock:Create(pos)
	function block:OnUse(player)
		player:OnWeaponUsed()
	end
	WeaponBlocksRegistry[i] = block
end

function Game.Player:OnWeaponUsed()
	Scheduler:Add(0.05, function(targetPlayer)
        if targetPlayer ~= nil then
            targetPlayer:RemoveWeapon()
            targetPlayer.model = Game.MODEL.NORMAL_ZOMBIE
            targetPlayer.model = Game.MODEL.DEFAULT
        end
    end, self)
end
...
function Game.Rule:OnGetWeapon(player, weaponid, weapon)
	local correctionFactor = Common.GetWeaponOption(weaponid).user.correctionFactor
	weapon.infiniteclip = true
	
	weapon.speed = weapon.speed * correctionFactor
end
...
function Game.Player:Init()
	...
	self.user.pixelwalk = false
	self.user.weaponModeIndex = 1
	-- mpbhop
	if MAP.Mode == 'mpbhop' then
		self:ResetLevel()
	end
...
if not DebugMode or StartButton then
	function StartButton:OnUse(player)
		if not player.user.pause then
			-- 퍼즈 중 타이머 시작 불가
			player.user.starttime = Game.GetTime()
...
function Game.Player:AngleGoCheck()
	local angle = self.user.angle or 0
	ChangeViewAngle(self, angle)
	stuck = stuck or 0
...
angleYaw = SyncValueCreate('angleYaw', VALUE.MaxPlayer)
function Game.Player:SetViewAngleYaw()
	local vector = self.velocity
	local function normalize2D(vector)
		local length = math.sqrt(vector.x^2 + vector.y^2)
		return {x=vector.x/length, y=vector.y/length}
	end
	self.user.viewAngleMode = true
	local angle = math.atan(normalize2D(vector).y, normalize2D(vector).x)
	local degree = angle * 180 / math.pi
	self.user.angle = degree
	
	angleYaw[self.index].value = degree
end
...
if Game.Rule:CanSave() then
	...
	function saveRecord(player, record)
		...
		if (playerBestLAP == nil or playerBestLAP > record) then
...
if MAP.Mode == 'mpbhop' then
	...
	for level, v in pairs(BackAndPoint) do
		...
		if v.TpZone then
			-- 다중 TPZone 처리
			if type(v.TpZone[1]) == 'table' then
				for j, k in ipairs(v.TpZone) do
					TpZone[#TpZone + 1] = {
						pos = sortMinMax(k),
...
Module = {}
Module.Scheduler = {}
Module.Scheduler.__index = Module.Scheduler

function Module.Scheduler:New()
...
function Module.Scheduler:Add(delay, callback, ...)
    local args = {...} -- 매개변수
    local task = {
        delay = delay,
        callback = callback,
...
function Module.Scheduler:Cancel(index)
    table.remove(self.tasks, index)
end

function Module.Scheduler:UpdateTasks(time)
    for i = #self.tasks, 1, -1 do
        ...
        -- 시간 확인
        if time - task.startTime >= task.delay then
...
Scheduler = Scheduler or Module.Scheduler:New()

function ChangeViewAngle(player,angle)
    print(player.index)
    player:SetThirdPersonFixedView(angle, 0, 0, -16)

    Scheduler:Add(0.05, function(targetPlayer)
        if targetPlayer ~= nil then
            targetPlayer:SetFirstPersonView()
        end
    end, player)
end
--------------------------------------------------------------------------------------------------
...
function Game.Player:OnSignalTime()
	...
	Scheduler:UpdateTasks(Game.GetTime())
end
...
function Game.Player:OnGameTime()
	...
	if MAP.BlockPixelWalk and self.velocity.z <= -3.5 and self.velocity.z >= -6.5 and (self.velocity.x == 0 or self.velocity.y == 0) then
		-- 서프에서 벽낑 방지 방어로직
		if isSurf(table.unpack(self.position)) or isSurf(self.position.x, self.position.y, self.position.z-1) then
			return
		end
		-- 벽낑 방지
		if self.user.pixelwalk then
			self:Signal(SIGNAL.ToUI.PIXELWALK)
		else
			self.user.pixelwalk = true
		end
	else
		self.user.pixelwalk = false
	end
...
function Game.Player:Pause_OFF()
	...
	Pause[self.index].value = false
	self:Signal(SIGNAL.ToUI.PAUSE_OFF)
end
...
function Game.Player:TeleportToPlayer(index)
	if Game.GetTime() - self.user.starttime > 1 then
		self.position = KZ.Player[index].position
	end
end

function Game.Player:BuyWeapon()
	self:ShowBuymenu()
end

SyncWeaponModeIndex = SyncValueCreate('weaponModeIndex', VALUE.MaxPlayer)

function Game.Player:SetWeaponMode()
	self.user.weaponModeIndex = self.user.weaponModeIndex + 1
	if self.user.weaponModeIndex > #WEAPONSPEED then
		self.user.weaponModeIndex = 1
	end
	SyncWeaponModeIndex[self.index].value = self.user.weaponModeIndex
	self.maxspeed = WEAPONSPEED[self.user.weaponModeIndex]
end
...
KZ.Event[SIGNAL.ToGame.SAVE_VIEW_ANGLE] = KZ.Event:Create(Game.Player.SetViewAngleYaw)
KZ.Event[SIGNAL.ToGame.ANGLE_CHECK_POINT] = KZ.Event:Create(Game.Player.AngleGoCheck)
KZ.Event[SIGNAL.ToGame.BUY_WEAPON] = KZ.Event:Create(Game.Player.BuyWeapon)
KZ.Event[SIGNAL.ToGame.SET_WEAPON_MODE] = KZ.Event:Create(Game.Player.SetWeaponMode)
...
function Game.Rule:OnPlayerJoiningSpawn(player)
	...
	player.user.viewAngleMode = false
...
-- CSOEPS_LUA_ISSUE_TOKEN:lua_7e3c97f0f8e44b51ba844932220497c4
